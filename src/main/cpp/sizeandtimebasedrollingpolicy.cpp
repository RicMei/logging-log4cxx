...
IMPLEMENT_LOG4CXX_OBJECT(SizeAndTimeBasedRollingPolicy)

struct SizeAndTimeBasedRollingPolicy::SizeAndTimeBasedRollingPolicyPrivate {
#if LOG4CXX_HAS_MULTIPROCESS_ROLLING_FILE_APPENDER
	SizeAndTimeBasedRollingPolicyPrivate() :
		_mmap(nullptr),
		_file_map(nullptr),
		_lock_file(nullptr),
	...
	long maxFileSize = 1024*1024*10; // 10M
	// add activeFileName
...
#else
int SizeAndTimeBasedRollingPolicy::createMMapFile(const std::string&, LOG4CXX_NS::helpers::Pool&) {
	return 0;
}

bool SizeAndTimeBasedRollingPolicy::isMapFileEmpty(LOG4CXX_NS::helpers::Pool&) {
	return true;
}

void SizeAndTimeBasedRollingPolicy::initMMapFile(const LogString&, LOG4CXX_NS::helpers::Pool&) {}

int SizeAndTimeBasedRollingPolicy::lockMMapFile(int) {
	return 0;
}

int SizeAndTimeBasedRollingPolicy::unLockMMapFile() {
	return 0;
}

const std::string SizeAndTimeBasedRollingPolicy::createFile(const std::string&, const std::string&, LOG4CXX_NS::helpers::Pool&) {
	return "";
}
#endif

SizeAndTimeBasedRollingPolicy::SizeAndTimeBasedRollingPolicy() :
	m_priv(std::make_unique<SizeAndTimeBasedRollingPolicyPrivate>())
{
}

SizeAndTimeBasedRollingPolicy::~SizeAndTimeBasedRollingPolicy() {}

void SizeAndTimeBasedRollingPolicy::activateOptions(LOG4CXX_NS::helpers::Pool& pool)
{
	...
	{
		LogLog::warn(
			LOG4CXX_STR("The FileNamePattern option must be set before using SizeAndTimeBasedRollingPolicy. "));
...
LOG4CXX_NS::pattern::PatternMap SizeAndTimeBasedRollingPolicy::getFormatSpecifiers() const
{
	PatternMap specs;
 ...
 * {@inheritDoc}
 */
RolloverDescriptionPtr SizeAndTimeBasedRollingPolicy::initialize(
	...
	Pool& pool)
{
	...
	else
	{
		...
		//	noAction, noAction);
		return std::make_shared<RolloverDescription>(
			m_priv->activeFileName, append,
			noAction, noAction);
	}
}

RolloverDescriptionPtr SizeAndTimeBasedRollingPolicy::rollover(
	const   LogString& currentActiveFile,
	const   bool        append,
	Pool& pool)
{
	...
	//
	if (_roll_reason == ROLL_REASON::roll_reason_time){
		if (newFileName == m_priv->lastFileName){
			RolloverDescriptionPtr desc;
			return desc;
		}
		else{
			m_priv->lastFileName = m_priv->lastFileName + "." + std::to_string(_current_roll_index);
			_current_roll_index = 1;
		}
	}
	else {
		m_priv->lastFileName = m_priv->lastFileName + "." + std::to_string(_current_roll_index++);
	...
	//if (newFileName == m_priv->lastFileName)
	//{
	//	if (_roll_reason == ROLL_REASON::roll_reason_filesize)
	//	{
	//		m_priv->lastFileName = m_priv->lastFileName + "." + std::to_string(_current_roll_index++);
	//	}
	//	else
	...
	//	}
	//}
	//if (newFileName == m_priv->lastFileName && _roll_reason != ROLL_REASON::roll_reason_filesize)
	//{
	//	RolloverDescriptionPtr desc;
	//	return desc;
	//}
...
bool SizeAndTimeBasedRollingPolicy::isTriggeringEvent(
	Appender* appender,
	...
	size_t fileLength)
{
	...
	//if (fileLength >= m_priv->maxFileSize || Date::currentTime() > m_priv->nextCheck) {
	//	return true;
	//}
	//else if (m_priv->maxFileSize > 0 && fileLength >= m_priv->maxFileSize) {
	//	lastFileName = lastFileName + "." + std::to_stirng(backupIndex++)
	//}

	if (Date::currentTime() > m_priv->nextCheck)
	{
		_roll_reason = ROLL_REASON::roll_reason_time;
		return true;
	}
	if (fileLength >= m_priv->maxFileSize)
	{
		_roll_reason = ROLL_REASON::roll_reason_filesize;
		return true;
	}
	return false;
	//return fileLength >= m_priv->maxFileSize || Date::currentTime() > m_priv->nextCheck;
}

void SizeAndTimeBasedRollingPolicy::setMultiprocess(bool multiprocess) {
#if LOG4CXX_HAS_MULTIPROCESS_ROLLING_FILE_APPENDER
...
void SizeAndTimeBasedRollingPolicy::setOption(const LogString& option,
	const LogString& value)
{
		...
		LOG4CXX_STR("throwioexceptiononforkfailure")))
	{
		...
		LOG4CXX_STR("maxfilesize")))
	{
		m_priv->maxFileSize = OptionConverter::toFileSize(value, 1024*1024*10);
		LogLog::debug("SizeAndTimeBasedRollingPolicy set maxFileSize = " + std::to_string(m_priv->maxFileSize));
	}
	else if (StringHelper::equalsIgnoreCase(option, LOG4CXX_STR("activefilename"), LOG4CXX_STR("ACTIVEFILENAME")))
	{
		m_priv->activeFileName.assign(value.c_str());
		LogLog::debug("SizeAndTimeBasedRollingPolicy set activeFileName = " + m_priv->activeFileName);
	}
	else
	{
